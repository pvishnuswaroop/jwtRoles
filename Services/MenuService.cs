...
namespace QuickServeAPP.Services
{
    public class MenuService : IMenuService
    {
        ...
        {
            var menuItem = await _menuRepository.GetMenuByIdAsync(menuId); // Ensure you check if the menu exists
            if (menuItem == null)
            {
                return false; // Return false if the menu item does not exist
            }

            await _menuRepository.DeleteMenuAsync(menuId); // Perform deletion
            return true;
        }
